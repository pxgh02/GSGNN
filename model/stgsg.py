...
class Embedding_Model(nn.Module):
    def __init__(self, n_gc, n_gs, n_hid):
        ...
        self.mlp_gc1 = MLP(n_hid, n_hid, n_hid)
        self.mlp_gc2 = MLP(n_hid, n_hid, n_hid)
    ...
    def forward(self, g, nf_gc, nf_gs):
        with (g.local_scope()):
            ...
            g.nodes['gc'].data['x'] = self.mlp_gc1(gc_embed)
            ...
            out_fc = self.mlp_gc2(g.nodes['gc'].data['new_ccx'])
            return out_fc, gs_embed
...
class GSegmentDown_Model(nn.Module):
    def __init__(self, n_hid):
        ...
        self.mlp_gs = MLP(n_hid , n_hid * 2, n_hid)
    ...
    def forward(self, g, nf_gc0, nf_gc1, nf_gs):
        with (g.local_scope()):
            ...
            g.nodes['gs'].data['x'] = self.mlp_gs(nf_gs)
            ...
            out_fs = self.lin_gs(torch.cat([g.nodes['gs'].data['new_sx'], g.nodes['gs'].data['x']], dim=1))
...
class GSegmentDown_Model_NSK(nn.Module):
    def __init__(self, n_hid):
        super(GSegmentDown_Model_NSK, self).__init__()
        self.n_hid = n_hid
...
class GCellUp_Model_NSK(nn.Module):
    def __init__(self, n_hid):
        super(GCellUp_Model_NSK, self).__init__()
        self.n_hid = n_hid
...
class HMPB(nn.Module):
    def __init__(self, n_in, n_out):
        super(HMPB, self).__init__()
        self.n_in = n_in
        self.n_out = n_out

        self.mlp_gs = MLP(n_in, n_out, n_out)
        self.mlp_gc = MLP(n_in, n_out, n_out)

        self.mlp_msg_Gcc = MLP(n_out * 2, n_out * 2, n_out * 4 + 1)
        self.mlp_reduce_Gcc = MLP(n_out * 5, n_out)
        self.mlp_msg_Gc2s = MLP(n_out * 2, n_out * 2, n_out * 4 + 1)
        self.mlp_reduce_Gc2s = MLP(n_out * 5, n_out)
        self.mlp_msg_Gs2c = MLP(n_out * 2, n_out * 2, n_out * 4 + 1)
        self.mlp_reduce_Gs2c = MLP(n_out * 5, n_out)

        self.lin_gs = nn.Linear(n_out * 2, n_out)
        self.lin_gc = nn.Linear(n_out * 2, n_out)
    ...
    def forward(self, g, nf_gc, nf_gs):
        with (g.local_scope()):
            g.nodes['gc'].data['x'] = self.mlp_gc(nf_gc)
            g.nodes['gs'].data['x'] = self.mlp_gs(nf_gs)
            ...
            out_gs = self.lin_gs(torch.cat([g.nodes['gs'].data['new_sx'], g.nodes['gs'].data['x']], dim=1))
            g.nodes['gs'].data['x'] = out_gs
            ...
            g.nodes['gc'].data['x'] = self.lin_gc(torch.cat([g.nodes['gc'].data['new_cx'], g.nodes['gc'].data['x']], dim=1))
            ...
            out_gc = g.nodes['gc'].data['new_ccx']

            return out_gc, out_gs

class STSG(nn.Module):
    def __init__(self, n_gc, n_gs, n_hid, n_out_congestion):
        super(STSG, self).__init__()
    ...
    def forward(self, g):
        ...
        fgc4, fgs4 = self.gcum1(g, fgc3, fgs3, fgs2)
        fgc5, fgs5 = self.gcum2(g, fgc4, fgs4, fgs1)
        
        congestion_out = self.mlp_congestion(fgc5)
        congestion_out = self.lin_congestion(congestion_out)
        
        return congestion_out

class STSG_NSK(nn.Module):
    def __init__(self, n_gc, n_gs, n_hid, n_out_congestion):
        super(STSG_NSK, self).__init__()
                
        self.em = Embedding_Model(n_gc, n_gs, n_hid)
        
        self.gsdm1 = GSegmentDown_Model_NSK(n_hid)
        self.gsdm2 = GSegmentDown_Model_NSK(n_hid)
        
        self.gcum1 = GCellUp_Model_NSK(n_hid)
        self.gcum2 = GCellUp_Model_NSK(n_hid)
    ...
    def forward(self, g):
        ...
        fgc2, fgs2 = self.gsdm1(g, fgc1, fgs1)
        fgc3, fgs3 = self.gsdm2(g, fgc2, fgs2)
        
        fgc4, fgs4 = self.gcum1(g, fgc3, fgs3)
        fgc5, fgs5 = self.gcum2(g, fgc4, fgs4)
...
class STSG2(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STSG2, self).__init__()

        self.em = Embedding_Model(n_gc, n_gs, 32)

        self.hmpb1 = HMPB(32, 48)
        self.hmpb2 = HMPB(48, 64)
        self.hmpb3 = HMPB(64 + 48, 48)
        self.hmpb4 = HMPB(48 + 32, 32)
        
        self.mlp_reduce = MLP(32, 32, 32)
        self.lin_reduce = nn.Linear(32, 1)
    ...
    def forward(self, g):
        ...
        fgc2, fgs2 = self.hmpb1(g, fgc1, fgs1)
        fgc3, fgs3 = self.hmpb2(g, fgc2, fgs2)
        fgc4, fgs4 = self.hmpb3(g, torch.cat([fgc3, fgc2], dim=1), torch.cat([fgs3, fgs2], dim=1))
        fgc5, fgs5 = self.hmpb4(g, torch.cat([fgc4, fgc1], dim=1), torch.cat([fgs4, fgs1], dim=1))

        congestion_out = self.mlp_reduce(fgc5)
        congestion_out = self.lin_reduce(congestion_out)
        
        return congestion_out

class STSG2_ZERO_GS(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STSG2_ZERO_GS, self).__init__()
...
class STSG2_ZERO_Capacity(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STSG2_ZERO_Capacity, self).__init__()
...
class STSG2_ZERO_NetDensity(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STSG2_ZERO_NetDensity, self).__init__()
...
class STSG2_ZERO_NPIN(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STSG2_ZERO_NPIN, self).__init__()
...
class STSG2_ZERO_PINDENSITY(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STSG2_ZERO_PINDENSITY, self).__init__()
...
class STSG2_ZERO_GS_Capacity_NPIN_PINDENSITY(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STSG2_ZERO_GS_Capacity_NPIN_PINDENSITY, self).__init__()
...
class STSG2_ZERO_GS_NetDensity(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STSG2_ZERO_GS_NetDensity, self).__init__()
...
class STGSG2d1(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STGSG2d1, self).__init__()

        self.em = Embedding_Model(n_gc, n_gs, 32)
        # self.em = Embedding_Model(n_gc, n_gs, 16)
        ...
        # self.hmpb1 = HMPB(16, 16)
        # self.hmpb2 = HMPB(16, 16)
        # self.hmpb3 = HMPB(16 + 16, 16)
        # self.hmpb4 = HMPB(16 + 16, 16)
    ...
    def forward(self, g):
        ...
        congestion_out = self.lin_reduce(fgc5)
        
        return congestion_out

class STGSG2d1_NO_VS(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STGSG2d1_NO_VS, self).__init__()
...
class STGSG2d1_NO_VSVCPIN(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STGSG2d1_NO_VSVCPIN, self).__init__()
...
class STGSG2d1_NO_VC_PIN(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STGSG2d1_NO_VC_PIN, self).__init__()
...
class STGSG2d1_NO_VC_PINDENSITY(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STGSG2d1_NO_VC_PINDENSITY, self).__init__()
...
class STGSG2d1_NO_VC_NETDENSITY(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STGSG2d1_NO_VC_NETDENSITY, self).__init__()
...
class STGSG2d1_ONLY_VC_NETDENSITY(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STGSG2d1_ONLY_VC_NETDENSITY, self).__init__()
...
class STGSG2d1_ONLY_VC_PINPINDensity(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STGSG2d1_ONLY_VC_PINPINDensity, self).__init__()
...
class STGSG2d5(nn.Module):
    def __init__(self, n_gc, n_gs):
        super(STGSG2d5, self).__init__()
        ...
        self.mlp_reduce = MLP(32, 32, 32)
        self.lin_reduce = nn.Linear(32, 2)
