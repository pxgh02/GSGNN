...
class Embedding_Model(nn.Module):
    def __init__(self, n_gc, n_gs, n_hid):
        ...
        self.mlp_msg_Gs2c = MLP(n_hid * 2, n_hid * 2, n_hid * 2 + 1)
        self.mlp_reduce_Gs2c = MLP(n_hid * 3, n_hid)

        self.mlp_msg_Gc2s = MLP(n_hid * 2, n_hid * 2, n_hid * 2 + 1)
        self.mlp_reduce_Gc2s = MLP(n_hid * 3, n_hid)
    ...
    def msg_c2s(self, edges):
        ...
        k, f1, f2 = torch.split(x, [1, self.n_hid, self.n_hid], dim=1)
        k = torch.sigmoid(k)
        return {'efso1': f1 * k, 'efso2': f2 * k}
    
    def reduce_c2s(self, nodes):
        x = torch.cat([nodes.data['x'], nodes.data['nfso1'], nodes.data['nfso2']], dim=1)
...
class GSegmentDown_Model(nn.Module):
    def __init__(self, n_hid):
        ...
        self.mlp_msg_Gc2s = MLP(n_hid * 3, n_hid * 2, n_hid * 2 + 1)
        self.mlp_reduce_Gc2s = MLP(n_hid * 3, n_hid)
        
        self.mlp_msg_Gss = MLP(n_hid * 2, n_hid * 2, n_hid * 2 + 1)
        self.mlp_reduce_Gss = MLP(n_hid * 3, n_hid)
    ...
    def msg_ss(self, edges):
        ...
        k, f1, f2 = torch.split(x, [1, self.n_hid, self.n_hid], dim=1)
        k = torch.sigmoid(k)
        return {'efsso1': f1 * k, 'efsso2': f2 * k}
    
    def reduce_ss(self, nodes):
        x = torch.cat([nodes.data['x'], nodes.data['nfsso1'], nodes.data['nfsso2']], dim=1)
...
class GCellUp_Model(nn.Module):
    def __init__(self, n_hid):
        ...
        self.mlp_msg_Gs2c = MLP(n_hid * 3, n_hid * 2, n_hid * 2 + 1)
        self.mlp_reduce_Gs2c = MLP(n_hid * 3, n_hid)
        
        self.mlp_msg_Gcc = MLP(n_hid * 2, n_hid * 2, n_hid * 2 + 1)
        self.mlp_reduce_Gcc = MLP(n_hid * 3, n_hid)
    ...
    def reduce_cc(self, nodes):
        x = torch.cat([nodes.data['x'], nodes.data['nfcco1'], nodes.data['nfcco2']], dim=1)
