...
def save_list_pickle(my_list, filename):
...
def avg_OF_result(tensor_input, is_plot=False):
    ...
    for y in range(gcell_length):
        for x in range(gcell_length - 1):
            avg_ef_horizontal[y][x] = (map_2d_horizontal[y*(gcell_length-1)*2+x*2] + map_2d_horizontal[y*(gcell_length-1)*2+x*2+1])/2
            if avg_ef_horizontal[y][x] > 0: #use BCEWithLogitsLoss so the threshold is 0
                avg_ef_horizontal[y][x] = 1
            else:
                avg_ef_horizontal[y][x] = 0
            
    # vertical
    for x in range(gcell_length):
        for y in range(gcell_length - 1):
            avg_ef_vertical[y][x] = (map_2d_vertical[x*(gcell_length-1)*2+y*2] + map_2d_vertical[x*(gcell_length-1)*2+y*2+1])/2
            if avg_ef_vertical[y][x] > 0: 
                avg_ef_vertical[y][x] = 1
            else:
                avg_ef_vertical[y][x] = 0

    return avg_ef_horizontal, avg_ef_vertical

def plot_density_map(data_h, data_v, title, save_dir=None):
    """
    绘制水平和垂直方向的密度图
    Args:
        data_h: shape为(gcell_len, gcell_len-1)的水平方向密度数据
        data_v: shape为(gcell_len-1, gcell_len)的垂直方向密度数据
        title: 图表标题
        save_dir: 保存图片的目录路径，如果为None则只显示不保存
    """
    plt.figure(figsize=(12, 5))
    ...
    # Plot horizontal map
    plt.subplot(1, 2, 1)
    ax1 = sns.heatmap(data_h, cmap='YlGnBu', annot=False, fmt='.2f', cbar=True)
    plt.gca().invert_yaxis()
    plt.title('Horizontal ' + title)
    plt.xlabel('X Coordinate')
    plt.ylabel('Y Coordinate')
    ...
    # Plot vertical map
    plt.subplot(1, 2, 2)
    ax2 = sns.heatmap(data_v, cmap='YlGnBu', annot=False, fmt='.2f', cbar=True)
    plt.gca().invert_yaxis()
    plt.title('Vertical ' + title)
    plt.xlabel('X Coordinate')
    plt.ylabel('Y Coordinate')
    ...
    if save_dir:
        ...
        os.makedirs(save_dir, exist_ok=True)
        # Replace spaces with underscores in title for filename
        filename = title.replace(' ', '_') + '_density_map.png'
        save_path = os.path.join(save_dir, filename)
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"Plot saved to {save_path}")
